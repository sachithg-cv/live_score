...
export class MatchScoreComponent implements OnInit {
  ...
  endInningModalVisble: boolean = false;
  ...
  matchCompleteForm = new FormGroup({
  ...
  openEndInningModal(): void {
    this.endInningModalVisble = true;
  }

  closeEndInningModal(): void {
    this.matchCompleteForm.reset({
      result: '',
    });
    this.endInningModalVisble = false;
  }

  submitMatchForm(): void {
    if (this.matchCompleteForm.invalid) {
      return;
    }
    const { result } = this.matchCompleteForm.value;
    this.matchService.completeMatch(this.match, { result })
      .pipe(takeUntil(this.notifier))
      .subscribe((_) => {
  ...
  endInning(): void {
    this.matchService.endFirstInning(this.match)
      .pipe(takeUntil(this.notifier))
      .subscribe((_) => {
        this.router.navigateByUrl('/matches/list');
      });
